...
(function(gdjs2) {
  ...
  const TweenRuntimeBehavior2 = class extends gdjs2.RuntimeBehavior {
    ...
    addObjectColorTween(identifier, toColorStr, easingValue, durationValue, destroyObjectWhenFinished, useHSLColorTransition) {
      const that = this;
      if (!this._isActive) {
        return;
      }
      ...
      const toColor = toColorStr.split(";");
      if (toColor.length !== 3) {
        return;
      }
      const newTweenable = TweenRuntimeBehavior2.makeNewTweenable(this._runtimeScene);
      if (useHSLColorTransition) {
        const fromColorAsHSL = rgbToHsl(parseFloat(fromColor[0]), parseFloat(fromColor[1]), parseFloat(fromColor[2]));
        const toColorAsHSL = rgbToHsl(parseFloat(toColor[0]), parseFloat(toColor[1]), parseFloat(toColor[2]));
        newTweenable.setConfig({
          from: {
    ...
    addObjectColorHSLTween(identifier, toHue, animateHue, toSaturation, toLightness, easingValue, durationValue, destroyObjectWhenFinished) {
      const that = this;
      if (!this._isActive) {
        return;
      }
      ...
      const toS = toSaturation === -1 ? fromColorAsHSL[1] : Math.min(Math.max(toSaturation, 0), 100);
      const toL = toLightness === -1 ? fromColorAsHSL[2] : Math.min(Math.max(toLightness, 0), 100);
      const newTweenable = TweenRuntimeBehavior2.makeNewTweenable(this._runtimeScene);
      newTweenable.setConfig({
        from: {
          hue: fromColorAsHSL[0],
